    ...
    link: function (scope, elem, attrs, ngModel) {
      ...
      scope.createContact = function () {
        ...
        var contactInfo = {
          ...
          'emails': scope.user.email ? [{ 'value': scope.user.email, 'type': 'business' }] : null,
          'organizations': scope.user.company ? [{'type': 'Company', 'name': scope.user.company}] : null,
          'photos': [{'value': scope.user.avatarUrl}],
          'birthday': Date('5/5/1973')
        };

        $cordovaContacts.save(contactInfo)
